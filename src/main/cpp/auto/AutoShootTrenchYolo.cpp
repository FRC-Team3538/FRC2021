...
std::string AutoShootTrenchYolo::GetName()
{
    return "7 - AutoShootTrenchYolo";
}
...
AutoShootTrenchYolo::~AutoShootTrenchYolo() {}

//State Machine
void AutoShootTrenchYolo::NextState()
{
    m_state++;
    m_autoTimer.Reset();
    m_autoTimer.Start();
...
void AutoShootTrenchYolo::Run()
{
    ...
    case 4:
    {
        IO.drivebase.DriveForward(-185.0, 0.8);
        IO.shooter.SetIntake(0.5);
        IO.shooter.SetIndexer(0.5);
        if (IO.drivebase.GetEncoderPositionLeft() < -185.0)
        {
            NextState();
        }
        break;
...
void AutoShootTrenchYolo::UpdateSmartDash()
{
